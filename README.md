...
##### 1.5.0
- 상급 재련 단계 표시 기능 추가
- 상급 재련 표시에 따른 버그 수정 : 상급 재련 단계 기능이 존재하지 않았을 때 아이템의 세트 정보를 표시 할 수 없었던 사항 수정
- 초월 및 세트아이템 레벨, 엘릭서 단계, 장신구 능력치옵션, 장신구 증감옵션의 가시성 향상
- 장비 화면의 가로 표시 영역 확대
- 캐릭터 상세 화면의 탭 바 불투명도 추가
##### 1.5.1
- 버그 수정 : 팔찌 옵션 중 능력치 증가 옵션 외 다른 옵션이 존재하지 않을 때 앱이 종료되던 버그 수정
##### 1.5.1
[기능 업데이트]

- 장착한 장비의 상세 시트에서 에스더 무기의 경우 엘라 부여 효과를 표시 할 수 있도록 하였습니다.
- 장착한 장비의 상세 시트에서 상급 재련 단계를 표시 할 수 있도록 하였습니다.


[인터렉션 업데이트] 

- 캐릭터 검색을 성공적으로 완료하거나 캐릭터 상세 페이지 내에서 메뉴를 이동 할 때 햅틱 인터랙션을 추가하였습니다.
- 캐릭터 상세 페이지 내에서 메뉴를 이동 할 때 선택한 메뉴의 컬러가 변경 되도록 하였습니다.


[UI 업데이트]

- 초월 단계를 표시하는 위치를 장비 이름의 하단 섹션으로 이동 하였습니다.
- 장착한 아이템의 등급에 따라 표시되던 그림자를 제거 하였습니다.
- 스킬 아이콘의 좌측에 존재하는 룬 아이콘, 스킬 이름, 스킬 레벨을 정렬 하였습니다.
- 장착 각인 아이콘의 부자연스러운 UI 를 수정하였습니다.
- 스킬 또는 수집포인트 등 다른 메뉴로 이동 할 때 캐릭터의 외형을 표시하는 섹션의 사이즈가 의도치 않게 변경되던 사항을 수정하였습니다.
- iPhone 8 Plus 이하에서 부자연스러운 UI 를 개선하였습니다.
- 장착 카드의 가로 세로의 비율을 실제 인게임와 동일한 비율로 수정하였습니다.
- 장착한 장비의 품질, 카드의 각성 단계를 표시하는 서브 뷰의 UI 를 수정하였습니다.


[버그 수정]

- 장비 상세 시트에서 엘라 부여가 완료된 에스더 무기를 표시 할 때 엘릭서 옵션이 두번 출력되던 사항을 수정하였습니다.


[성능 개선]

- 캐릭터 검색과 관련한 성능을 개선하였습니다.
</details>
...
- 2023년 10월 25일 출시부터 2024년 4월 10일 기준으로 앱 노출 수 4.9만, 페이지 조회수 3600건, 컨버전률 1.97%, 전체 다운로드 수 661 건의 LOAS 는 제 첫 출시 프로젝트로 제가 너무나도 좋아하는 게임과 관련된 앱을 출시함으로서 현재까지 약 10건의 업데이트를 진행했습니다.
