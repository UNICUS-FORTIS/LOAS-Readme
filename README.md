...
- 장착한 장비의 상세 시트에서 에스더 무기의 경우 엘라 부여 효과를 표시
- 장착한 장비의 상세 시트에서 상급 재련 단계를 표시


[인터렉션 업데이트] 

- 캐릭터 검색을 성공적으로 완료하거나 캐릭터 상세 페이지 내에서 메뉴를 이동 할 때 햅틱 인터랙션 추가
- 캐릭터 상세 페이지 내에서 메뉴를 이동 할 때 선택한 메뉴의 컬러가 변경 되도록 수정


[UI 업데이트]

- 초월 단계를 표시하는 위치를 장비 이름의 하단 섹션으로 이동
- 장착한 아이템의 등급에 따라 표시되던 그림자를 제거
- 스킬 아이콘의 좌측에 존재하는 룬 아이콘, 스킬 이름, 스킬 레벨을 정렬
- 장착 각인 아이콘의 부자연스러운 UI 를 수정
- 스킬 또는 수집포인트 등 다른 메뉴로 이동 할 때 캐릭터의 외형을 표시하는 섹션의 사이즈가 의도치 않게 변경되던 사항을 수정
- iPhone 8 Plus 이하에서 부자연스러운 UI 를 개선
- 장착 카드의 가로 세로의 비율을 실제 인게임와 동일한 비율로 수정
- 장착한 장비의 품질, 카드의 각성 단계를 표시하는 서브 뷰의 UI 를 수정


[버그 수정]

- 장비 상세 시트에서 엘라 부여가 완료된 에스더 무기를 표시 할 때 엘릭서 옵션이 두번 출력되던 사항을 수정
